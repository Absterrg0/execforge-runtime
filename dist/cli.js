...
async function main() {
    ...
    if (command === "finish") {
        ...
        if (!posted)
            process.exitCode = 0; // never break CI due to telemetry failure
        return;
    }
    if (command === "run") {
        const wrapped = argsAfterDoubleDash();
        if (!wrapped) {
            throw new Error("Usage: execforge run -- \"npm test\"\n" +
    ...
    console.log("ExecForge Runtime CLI v" + (await import("./config.js").then(m => m.COLLECTOR_VERSION)) + "\n\n" +
        "Usage:\n" +
