...
function appendCommandFile(envVar, message) {
...
async function handleFinish(config, exitCode) {
 ...
 */
async function handleAutoPost(config) {
    ...
    if (!jobStatus) {
        console.log("[ExecForge] Tip: add  env: EXECFORGE_JOB_STATUS: ${{ job.status }}  to your action step\n" +
...
async function main() {
    ...
    if (mode === "run") {
        ...
        if (!command) {
            throw new Error("The 'command' input is required when mode=run.\n" +
